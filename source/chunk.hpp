...
enum ChunkState : uint8_t {
  ...
  WAITING_FOR_UPLOAD =
      2,         // Mesh generated, waiting to be sent to GPU or binded to VAO
  UPLOADING = 3, // Being uploaded or drawn
  IDLE = 4,      // Fully loaded
  MARKED_FOR_DELETION =
      5, // Waiting for all references to be released before deletion
...
private:
  ...
  std::vector<Voxel::PackedVoxel>
      meshData[4];            // One per LOD level (vertex data only)
  ...
  void greedyMeshAxisLOD(std::vector<Voxel::PackedVoxel> &meshData,
                         ...
                         int height, int depth, int voxelScale, int d, int u,
                         int v, int direction, Voxel::VoxelFace faceDir);
  bool shouldRenderFaceLOD(uint8_t downsampledVoxels[16][16][16], int x, int y,
                           int z, int d, int direction, int width, int height,
                           int depth) const;
...
public:
  ...
  size_t getVertexCount(LODLevel lod) const {
    return lodMeshNeedsUpdate[lod] ? 0 : meshData[lod].size();
  }
